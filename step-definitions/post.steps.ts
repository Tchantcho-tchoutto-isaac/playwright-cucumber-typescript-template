...
Then('JE remplit le formuaire avec les informations suivantes  titre {string}  contenue {string} et je soumet', async (s: string, s2: string) => {
  // Write code here that turns the phrase above into concrete actions
  postPage = new PostPage(page);
  await postPage.fillTitle(s);
  await postPage.fillContent(s2);
  await postPage.clickSave();
})

Then('je suis redirigé vers la page admin et le message de succes s affiche', async () => {
  // Write code here that turns the phrase above into concrete actions
  const successMessage = await page.locator('.success'); // Remplace par le bon sélecteur CSS
